  ...
  return (
    <section id="newsletter-archive" className="mb-12">
      <div className="bg-headingBg inline-flex items-center gap-3 px-4 py-2 rounded mb-6">
        <h1 className="text-3xl font-medium text-mainDark">{data.title}</h1>
      ...
      <div className="list-disc  space-y-3 space-x-3">
        {data.newsletters.map((newsletter, i) => (
          <a
            ...
            className="inline-block px-4 py-2 border shadow-lg border-main hover:border-mainDark rounded-full text-main hover:bg-blue-50 transition-colors text-md hover:text-mainDark font-medium"
          >
            {newsletter.text}
          </a>
        ))}
